...
This gives us the compactness scores from each of our three metrics in the same df, ready to be mapped. We also still retain the three seperate geometries used to calculate area for the compactness metrics, which will be potentially useful down the road. Here's the resulting table: 
...
Now, conducting the same style intersection -> area weighting -> re-aggregation for precinct data, saving back the democratic vote share as a percentage. 
...
gerrymander_data<- gerrymander_data%>%
  mutate(difference_convex = district_pop$black_pct-chull_pop$black_pct,
         difference_minimum_bounding = `district_pop$black_pct`- bound_pop$black_pct)
...
The calculated compactness metrics have a somewhat inverse relationship with the percent democratic vote, indicating that gerrymandering is clustering democratic votes together in similar districts. 
...
cor_matrix <- gerrymander_data |> 
  ...
  select(`Percent Black Within District`,
         `Difference between CHull and District`, 
...
gerrymander_data_plot_race <- gerrymander_data |> 
  st_drop_geometry() |> 
  select(District, `Percent Dem Votes in District`, `Difference between CHull and District`, `Polsby-Popper`, `Convex Hull`,  `Minimum Bounding Circle`) |> 
